...
1. `compgen <configPath>`: Autogenerate Obelisk schemas based on the store components config file
...
This tool will use the `obelisk.config.ts` to detect all systems, components, singletonSchemas, and projectName in the World and will deploy them to the chain specified.
...
When you add a new component or modify the system code, you need to upgrade the contract through the `upgrade` method. ([Contract upgrade specification](../migrating-from-others))

```bash
obelisk publish --configPath obelisk.config.ts
```
