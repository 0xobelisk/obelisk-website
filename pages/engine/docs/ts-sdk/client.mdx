...
We create a Project called Counter and declare sigletonSchemas called counter, which is of type `u64` and has an initial value of `0`.
...
List all component name in the world store.
...
const componentNames = await obelisk.listComponentNames(
...
Get all the entities in the world by component name.
...
const componentName = Object.keys(obeliskConfig.components)[0]

const entities = await obelisk.getEntities(
    worldId,
    componentName
);
```
...
Get the entity's data based on component name and entity id(option).
...
// get singleton component entity data
const singletonComponentName = Object.keys(obeliskConfig.singletonSchemas)[0]
const entities = await obelisk.getEntity(
    worldId,
    componentName
);
