...
To create schemas in Obelisk we are going to navigate to the `dubhe.config.ts` file. You can define schemas, their types, their values, and other types of information here. Obelisk then autogenerates all of the files needed to make sure your app knows these schemas exist.
...
```tsx filename="dubhe.config.ts" {8-16} copy showLineNumbers
...
```tsx filename="Map.ts"
    ...
    // dubhe.tx.map_system.move_t(stepTxB, params, undefined, true);
    (await dubhe.tx.map_system.move_t(stepTxB, params, undefined, true)) as TransactionResult;
}
