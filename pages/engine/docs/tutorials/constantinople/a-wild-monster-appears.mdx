...
<CollapseCode>
```tsx filename="dubhe.config.ts" {51-59} copy showLineNumbers
...
```tsx filename="dubhe.config.ts" {60-63} copy showLineNumbers
import { DubheConfig } from '@0xobelisk/common';
...
let params = [tx.object(WORLD_ID), tx.pure.address('0x6')];
(await dubhe.tx.encounter_system.throw_ball(tx, params, undefined, true)) as TransactionResult;
...
let tx = new Transaction();
let params = [tx.object(WORLD_ID)];
(await dubhe.tx.encounter_system.flee(tx, params, undefined, true)) as TransactionResult;
