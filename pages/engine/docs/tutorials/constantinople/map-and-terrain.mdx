...
Go ahead and add the `map` as a singleton schema in the Obelisk config (`dubhe.config.ts`).

<CollapseCode>

```tsx filename="dubhe.config.ts" {8-47} copy showLineNumbers
...
````tsx filename="dubhe.config.ts" {48} copy showLineNumbers
import { DubheConfig } from '@0xobelisk/common';
